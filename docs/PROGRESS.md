...
- [x] 产出 above 对比图复用能力与页面优化设计文档（2026-03-23）
- [x] 产出 above 对比图复用能力与页面优化实施计划（2026-03-23）
...
- [x] 产出 as 总览型词条页设计文档（2026-03-30）
- [x] 产出 as 总览型词条页实施计划（2026-03-30）
- [x] 产出 since 词条 SEO 与详情页稳定性设计文档（2026-03-31）
- [x] 产出 since 词条 SEO 与详情页稳定性实施计划（2026-03-31）
- [x] 将“多义词条优先在现有文本模块统一解释、保持详情页结构稳定”的规则写入 PRD（2026-03-31）
- [x] 产出 according to 中文页内容优化修改计划（2026-06-01）
- [x] 执行 according to 中文页内容优化：释义融入 accord/to 记忆线、差异解析改为 in my opinion/about/due to、增加专属决策树、修正微测验第 3 题为信息来源考察。（2026-06-01）
- [x] 产出 due to 页面优化设计与实施计划：cause -> result 双立方体抽象示意图、中英文差异解析、专属决策树。（2026-06-01）
- [x] 优化 due to 词条：新增 cause -> result 双立方体抽象示意图，重写中英文差异解析，并加入专属决策树。（2026-06-01）
- [x] 微调 due to 中文页抽象示意图标签：中文 locale 下将 cause/result 显示为“原因/结果”，英文页保持 cause/result。（2026-06-01）
- [x] 同步 due to 首页封面图：新增与详情页一致的“原因 -> 结果”双立方体 SVG 缩略图，并纳入专项检查。（2026-06-01）
- [x] 产出 including 词条页面优化设计文档：限定为词条专属内容、决策树与首页 SVG 封面，不改通用组件。（2026-06-01）
- [x] 产出 including 词条页面优化实施计划：覆盖差异解析、专属决策树、首页封面 SVG、可选 scene 数据微调与最终校验。（2026-06-01）
- [x] 执行 including 词条页面优化：差异解析改为 apart from / except for / among，增加专属决策树，新增“整体容器内包含对象”的首页 SVG 封面，并微调详情页 scene 为透明容器内对象。（2026-06-01）
- [x] 同步 including 详情页示意图：详情页顶部视觉改为复用首页 `including.svg` 静态插画，并为静态示意图提供独立中英文控制栏文案。（2026-06-01）
- [x] 产出 instead of 词条页面优化设计文档：明确差异解析、专属决策树、详情页动态替换 SVG 与首页静态封面图方案，视觉风格对齐站内既有示意图。（2026-06-01）
- [x] 产出 instead of 词条页面优化实施计划：覆盖内容测试、差异解析、专属决策树、动态/静态 SVG 资源、详情页专属视觉接入与最终校验。（2026-06-01）
- [x] 执行 instead of 词条页面优化：差异解析改为 rather than / as / without，增加专属决策树，新增替换关系动态详情 SVG 与首页静态封面图。（2026-06-01）
- [x] 调整 instead of 详情页示意图风格：改用与 above 同源的共享 Three.js 3D renderer，使用透明线框立方体表现 A 移入并替换 B 的位置。（2026-06-01）
- [x] 调整 instead of 替换动画路径：A 从 B 的垂直方向进入中心位置，同时 B 被挤出到队列外。（2026-06-02）
- [x] 优化 including 示意图风格：保留“整体容器内包含对象”结构，并将 SVG 线条颜色、线宽与半透明度统一到现有缩略图线框风格。（2026-06-01）
- [x] 修正 including 详情页示意图线条粗细：SVG 线条改为不随详情页放大而变粗，并按现有 3D renderer 的线色与透明度对齐。（2026-06-01）
- [x] 修复 including 详情页示意图一致性：详情页不再使用静态 SVG，改回与 above 相同的共享 3D renderer，以保证尺寸、线条粗细、颜色和透明度同源。（2026-06-01）
...
- [x] 优化 including 透明容器可读性：大容器改用不写入深度的低透明面材质，并开启地面，使内部 4 个小立方体可见且风格接近 above。（2026-06-01）
- [x] 同步 including 首页封面：在 4 个内部小立方体缩略图中加入淡灰地面层，使首页卡片与详情页透明容器场景保持一致。（2026-06-01）
- [x] 修正 including 首页封面外层容器比例：按 above/详情页同源投影重画透明大立方体，避免最外层立方体变形。（2026-06-02）
...
- [x] 列表页：搜索、卡片网格、进入详情
- [x] 首页搜索策略升级：英文仅匹配介词词面；非英文优先词面、零结果时回退到卡片释义匹配。（2026-03-30）
- [x] 首页搜索结果收敛：搜索态仅显示有命中的分类板块，并将 Abstract 分类标题统一为介词式命名。（2026-03-30）
...
- [x] 优化 since 词条双语内容与 SEO 主轴：保持英文时间义聚焦，同时补齐“因为 / 既然”义项、since vs because 辨析与双义 FAQ。（2026-03-31）
- [x] 修复 since 词条决策树内容与 Common collocations 高亮：决策树补齐时间起点/原因义判断，Present perfect patterns 改为高亮 since 而非句首助动词。（2026-03-31）
- [x] 优化 since 抽象场景示意图：改为“原因节点 -> 结果节点”的双圆因果图，紫色小球固定在原因起点。（2026-03-31）
- [x] 简化 since 抽象场景节点样式：移除圆形叠加 outline，避免摄像机移动时出现线条闪烁。（2026-03-31）
- [x] 提升 since 抽象场景节点可见性：将原因/结果节点改为浅白偏紫填充，避免白底下难以辨认。（2026-03-31）
