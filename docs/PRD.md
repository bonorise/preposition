...
- 交互：
  - 搜索：
    - 英文界面按介词词面搜索
    - 非英文界面优先按介词词面搜索；若无结果，则按本地释义回退匹配
    - 搜索结果仅显示有命中的分类板块，避免空分类噪音
  - 标签筛选（可选：MVP 可先做搜索）
  - 点击进入详情页
...
- 每句包含：
  - 英文句子（简短，避免复杂从句）
  - 中文释义（贴合初学者）
- 若某个介词存在两个或以上高频义项，优先在现有 `meaning / tips / comparison / FAQ` 文本模块中解释这些义项共享的核心感觉，再分别展开常见用法；不要为了单个词条随意改变详情页总结构
- 后续扩展字段预留（MVP 不实现）：
  - 发音音频（tts_url）
  - 关键词高亮（tokens）
  - 可替换名词（templates）
...
- 中英文页面可以按搜索意图调整义项权重，但核心解释必须一致；不能出现英文与中文对同一介词给出互相冲突的语义主轴
